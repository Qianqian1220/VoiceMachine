...
# 1️⃣ 设定固定存储目录
output_dir = "results_3_acc"
os.makedirs(output_dir, exist_ok=True)

# 2️⃣ 加载标准答案 data_a.csv（注意：这里使用 data_a.csv）
ref_df = pd.read_csv("data_a.csv", dtype={"ID": str})  # 确保 ID 为字符串
ref_df.rename(columns={"ID": "uttid", "Sentence": "reference"}, inplace=True)
reference_texts = ref_df["reference"].tolist()

if len(reference_texts) != 609:
    raise ValueError(f"❌ 参考文本数量错误，应为 609，当前为 {len(reference_texts)}")

# 3️⃣ 遍历 result_1 目录中的所有 CSV 识别结果（这里目录改为 result_1）
asr_results_dir = "results_3"
...
for asr_file in asr_files:
    ...
    if len(asr_df) != 609:
        raise ValueError(f"❌ {asr_file} 句子数量错误，应为 609，当前: {len(asr_df)}")
    ...
    if len(asr_texts) != 609:
        raise ValueError(f"❌ 解析出的 ASR 结果数量错误，应为 609，当前: {len(asr_texts)}")
    ...
    # 6️⃣ 构建 609x609 的代价矩阵（cost matrix），每个元素为参考句与 ASR 句之间的 CER
    cost_matrix = np.zeros((609, 609))
    for i in range(609):
        for j in range(609):
    ...
    # 9️⃣ 生成 CER 结果文件
    # 保存匹配关系时，uttid 取 ASR 结果对应的 uttid（按照 col_ind 顺序排序）
